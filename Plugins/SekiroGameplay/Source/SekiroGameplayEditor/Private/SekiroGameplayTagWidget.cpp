 ...
 */
void SSekiroGameplayTagWidget::Construct(const FArguments& Arguments)
{
    ...
    [
        ...
        [
            ...
            [ SNew(SButton).Text(FText::FromString(TEXT("选择…"))).OnClicked(this, &SSekiroGameplayTagWidget::ChooseLuaFile) ]
        ]
        + SVerticalBox::Slot().AutoHeight().Padding(8, 4)
        [
            SNew(SHorizontalBox)
        ...
        [
            ...
            [ SNew(SButton).Text(FText::FromString(TEXT("预览标签"))).OnClicked(this, &SSekiroGameplayTagWidget::Preview) ]
            + SHorizontalBox::Slot().AutoWidth().Padding(8, 0)
            [ SNew(SButton).Text(FText::FromString(TEXT("生成 / 更新数据表"))).OnClicked(this, &SSekiroGameplayTagWidget::Generate) ]
            + SHorizontalBox::Slot().AutoWidth()
            [ SNew(SButton).Text(FText::FromString(TEXT("在内容浏览器中定位"))).OnClicked(this, &SSekiroGameplayTagWidget::BrowseAsset)
...
FReply SSekiroGameplayTagWidget::ChooseLuaFile()
{
...
FReply SSekiroGameplayTagWidget::Preview()
{
    TArray<FSekiroGameplayTagEntry> Tags;
    FString Error;
    if (!USekiroGameplayTagLibrary::PreviewGameplayTags(SourceInput->GetText().ToString(), Tags, Error))
    {
    ...
    for (const FSekiroGameplayTagEntry& Entry : Tags) Lines.Add(Entry.Tag + TEXT("    ") + Entry.Comment);
...
FReply SSekiroGameplayTagWidget::Generate()
{
    FSekiroGameplayTagGenerationResult Result;
    USekiroGameplayTagLibrary::GenerateGameplayTagTable(SourceInput->GetText().ToString(), AssetInput->GetText().ToString(), bRegisterSource, Result);
...
FReply SSekiroGameplayTagWidget::BrowseAsset()
{
...
void SSekiroGameplayTagWidget::InvalidatePreview()
{
