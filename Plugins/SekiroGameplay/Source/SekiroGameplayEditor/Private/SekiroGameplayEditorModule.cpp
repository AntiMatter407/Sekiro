#include "SekiroGameplayEditorModule.h"

#include "SekiroGameplayTagWidget.h"
...
namespace SekiroGameplayEditor
{
    const FName TabId(TEXT("SekiroGameplayTools"));
    ...
    {
        FPolyglotTextData Data(ELocalizedTextSourceCategory::Editor, TEXT("SekiroGameplayEditor"), Key, English, TEXT("en"));
...
IMPLEMENT_MODULE(FSekiroGameplayEditorModule, SekiroGameplayEditor)

/** 模块启动时注册内置工具、停靠窗口与菜单；游戏线程。命令行模式不访问 Slate，但公共生成接口仍可用。 */
void FSekiroGameplayEditorModule::StartupModule()
{
    if (IsRunningCommandlet()) return;
    MenuLabel = SekiroGameplayEditor::MakeMenuText(TEXT("RootMenu"), TEXT("LuaGameplay"), TEXT("Lua玩法"));
    MenuToolTip = SekiroGameplayEditor::MakeMenuText(TEXT("RootMenuToolTip"), TEXT("Lua gameplay authoring tools"), TEXT("Lua 玩法配置与资产生成工具"));
    const FText TagImportLabel = SekiroGameplayEditor::MakeMenuText(TEXT("GameplayTagImport"), TEXT("Import GameplayTags from Lua"), TEXT("GameplayTag 的 Lua 导入"));
    RegisterTool(SekiroGameplayEditor::TagToolId, TagImportLabel, FSekiroGameplayToolWidgetFactory::CreateLambda([]()
    {
        return StaticCastSharedRef<SWidget>(SNew(SSekiroGameplayTagWidget));
    }));
    FGlobalTabmanager::Get()->RegisterNomadTabSpawner(SekiroGameplayEditor::TabId, FOnSpawnTab::CreateRaw(this, &FSekiroGameplayEditorModule::SpawnToolsTab))
        .SetDisplayName(MenuLabel).SetMenuType(ETabSpawnerMenuType::Hidden);
    UToolMenus::RegisterStartupCallback(FSimpleMulticastDelegate::FDelegate::CreateRaw(this, &FSekiroGameplayEditorModule::RegisterMenus));
}

/** 卸载时移除菜单、窗口工厂及引用；游戏线程。扩展模块应先注销其工厂，防止卸载后回调。 */
void FSekiroGameplayEditorModule::ShutdownModule()
{
    if (!IsRunningCommandlet())
    {
        UToolMenus::UnRegisterStartupCallback(this);
        UToolMenus::UnregisterOwner(this);
        TSharedPtr<SDockTab> ExistingTab = FGlobalTabmanager::Get()->FindExistingLiveTab(SekiroGameplayEditor::TabId);
        if (ExistingTab) ExistingTab->RequestCloseTab();
        FGlobalTabmanager::Get()->UnregisterNomadTabSpawner(SekiroGameplayEditor::TabId);
    }
    RootContent.Reset();
    Tools.Reset();
}
...
FSekiroGameplayEditorModule& FSekiroGameplayEditorModule::Get()
{
    check(IsInGameThread());
    return FModuleManager::LoadModuleChecked<FSekiroGameplayEditorModule>(TEXT("SekiroGameplayEditor"));
}
 ...
 */
bool FSekiroGameplayEditorModule::RegisterTool(FName ToolId, const FText& DisplayName, FSekiroGameplayToolWidgetFactory Factory)
{
...
bool FSekiroGameplayEditorModule::UnregisterTool(FName ToolId)
{
...
void FSekiroGameplayEditorModule::OpenToolsWindow()
{
    if (!IsInGameThread() || IsRunningCommandlet()) return;
    FGlobalTabmanager::Get()->TryInvokeTab(SekiroGameplayEditor::TabId);
}
 ...
 */
void FSekiroGameplayEditorModule::OpenTool(FName ToolId)
{
    if (!IsInGameThread() || IsRunningCommandlet()) return;
    for (const FToolRegistration& Tool : Tools)
    {
...
void FSekiroGameplayEditorModule::RegisterMenus()
{
    ...
    FToolMenuEntry& Entry = Menu->FindOrAddSection(NAME_None).AddSubMenu(
        ...
        FNewToolMenuDelegate::CreateRaw(this, &FSekiroGameplayEditorModule::BuildToolMenu));
 ...
 */
void FSekiroGameplayEditorModule::BuildToolMenu(UToolMenu* Menu)
{
    ...
    for (const FToolRegistration& Tool : Tools)
    {
        Section.AddMenuEntry(Tool.Id, Tool.DisplayName, FText::GetEmpty(), FSlateIcon(),
            FUIAction(FExecuteAction::CreateRaw(this, &FSekiroGameplayEditorModule::OpenTool, Tool.Id)));
    }
}

/** 创建停靠页；游戏线程。Args 为 Slate 调用上下文，返回新窗口，内容生命周期由窗口管理。 */
TSharedRef<SDockTab> FSekiroGameplayEditorModule::SpawnToolsTab(const FSpawnTabArgs& Args)
{
...
void FSekiroGameplayEditorModule::RebuildToolsContent()
{
...
void FSekiroGameplayEditorModule::SelectTool(FName ToolId)
{
    ActiveTool = ToolId;
    RebuildToolsContent();
}
