...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FLuaAnimSnapshotLoaderTest,
    "Lua.AnimBlueprintExt.Snapshot.Loader.JsonLines",
 ...
 */
bool FLuaAnimSnapshotLoaderTest::RunTest(const FString& Parameters)
{
    ...
    FLuaAnimSnapshotDocument Document;
    FLuaAnimSnapshotLoader::ParseJsonLines(
        JsonLines,
    ...
    const TSharedPtr<FLuaAnimSnapshotFrame>& DetailedFrame = Document.Frames[1];
    TestTrue(
        TEXT("变化描述包含状态机变化"),
    ...
    TestTrue(
        TEXT("搜索可命中 Lua 动画名"),
        SLuaAnimSnapshotViewer::DoesFrameMatchSearchForTesting(
            DetailedFrame,
            TEXT("run_fwd")));
    TestTrue(
        TEXT("搜索可命中变量名"),
    ...
    const TSharedPtr<FLuaAnimSnapshotNode>& RootNode = DetailedFrame->Roots[0];
    ...
    {
        const FLuaAnimTransitionSample& Sample = DetailedFrame->Transitions[0];
    ...
    FLuaAnimSnapshotDocument RootPriorityDocument;
    FLuaAnimSnapshotLoader::ParseJsonLines(
