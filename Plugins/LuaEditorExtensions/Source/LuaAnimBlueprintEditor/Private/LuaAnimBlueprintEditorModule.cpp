...
DEFINE_LOG_CATEGORY_STATIC(LogLuaAnimBlueprintEditor, Log, All);

namespace LuaAnimBlueprintEditorPrivate
{
const FName LuaAnimSnapshotViewerTabName(TEXT("LuaAnimSnapshotViewer"));
}

/** 管理 Lua 动画源码监听、PIE 运行缓存边界以及官方动画蓝图编辑器工具栏扩展。 */
class FLuaAnimBlueprintEditorModule final : public IModuleInterface
{
public:
...
private:
    ...
    static void MarkPendingSourceChanges(
        TWeakPtr<FLuaAnimBlueprintAutoCompileScheduler, ESPMode::ThreadSafe> WeakScheduler);

    TSharedPtr<FLuaAnimBlueprintAutoCompileScheduler, ESPMode::ThreadSafe> Scheduler; // 事件驱动标脏请求合并器
    ...
    TArray<TSharedPtr<FLuaAnimBlueprintEditorBinding>> EditorBindings; // 每个命令列表唯一的安全绑定
 ...
 */
void FLuaAnimBlueprintEditorModule::StartupModule()
{
    bShuttingDown = false;

    FGlobalTabmanager::Get()->RegisterNomadTabSpawner(
        LuaAnimBlueprintEditorPrivate::LuaAnimSnapshotViewerTabName,
        FOnSpawnTab::CreateRaw(
            this,
            &FLuaAnimBlueprintEditorModule::SpawnLuaAnimSnapshotViewerTab))
    ...
    UToolMenus::RegisterStartupCallback(
        FSimpleMulticastDelegate::FDelegate::CreateRaw(
            this,
            &FLuaAnimBlueprintEditorModule::RegisterLuaAnimSnapshotViewerMenus));

    FLuaAnimBlueprintEditorBinding::PrepareEditorLuaDebugBeforeEnvCreation();
    ...
    if (UnLuaModule != nullptr)
    {
        ...
        UnLuaModule->GetEnv();
        if (!FLuaAnimBlueprintEditorBinding::ApplyEditorLuaDebugSetting())
        {
            UE_LOG(
                LogLuaAnimBlueprintEditor,
                Warning,
    ...
    Scheduler = MakeShared<FLuaAnimBlueprintAutoCompileScheduler, ESPMode::ThreadSafe>();
    RegisterScriptWatcher();
    if (GEditor != nullptr)
    {
        BlueprintPreCompileHandle = GEditor->OnBlueprintPreCompile().AddRaw(
            this,
            &FLuaAnimBlueprintEditorModule::HandleBlueprintPreCompile);
    }
    PreBeginPIEHandle = FEditorDelegates::PreBeginPIE.AddRaw(
        this,
        &FLuaAnimBlueprintEditorModule::HandlePreBeginPIE);
    EndPIEHandle = FEditorDelegates::EndPIE.AddRaw(
        this,
        &FLuaAnimBlueprintEditorModule::HandleEndPIE);
    ...
    IAnimationBlueprintEditorModule::FAnimationBlueprintEditorToolbarExtender ToolbarDelegate =
        IAnimationBlueprintEditorModule::FAnimationBlueprintEditorToolbarExtender::CreateRaw(
            this,
            &FLuaAnimBlueprintEditorModule::ExtendAnimationBlueprintToolbar);
 ...
 */
void FLuaAnimBlueprintEditorModule::ShutdownModule()
{
    bShuttingDown = true;
 ...
 */
TSharedRef<FExtender> FLuaAnimBlueprintEditorModule::ExtendAnimationBlueprintToolbar(
    const TSharedRef<FUICommandList> CommandList,
    TSharedRef<IAnimationBlueprintEditor> Editor)
{
    EditorBindings.RemoveAll([](
        const TSharedPtr<FLuaAnimBlueprintEditorBinding>& Candidate)
    {
        return !Candidate.IsValid() || !Candidate->IsValid();
    });

    TSharedPtr<FLuaAnimBlueprintEditorBinding> Binding;
    for (const TSharedPtr<FLuaAnimBlueprintEditorBinding>& Candidate : EditorBindings)
    {
    ...
    if (!Binding.IsValid())
    {
        Binding = FLuaAnimBlueprintEditorBinding::Create(CommandList, Editor);
 ...
 */
TSharedRef<SDockTab> FLuaAnimBlueprintEditorModule::SpawnLuaAnimSnapshotViewerTab(
    const FSpawnTabArgs& SpawnTabArgs)
{
    return SNew(SDockTab)
        .TabRole(ETabRole::NomadTab)
        [
            SNew(SLuaAnimSnapshotViewer)
        ];
}
 ...
 */
void FLuaAnimBlueprintEditorModule::RegisterLuaAnimSnapshotViewerMenus()
{
 ...
 */
void FLuaAnimBlueprintEditorModule::RegisterScriptWatcher()
{
    ...
    if (DirectoryWatcher == nullptr
        || !DirectoryWatcher->RegisterDirectoryChangedCallback_Handle(
            ...
            IDirectoryWatcher::FDirectoryChanged::CreateRaw(
                this,
                &FLuaAnimBlueprintEditorModule::HandleDirectoryChanged),
 ...
 */
void FLuaAnimBlueprintEditorModule::UnregisterScriptWatcher()
{
 ...
 */
void FLuaAnimBlueprintEditorModule::HandleDirectoryChanged(
    const TArray<FFileChangeData>& FileChanges)
{
    ...
    const TWeakPtr<FLuaAnimBlueprintAutoCompileScheduler, ESPMode::ThreadSafe>
 ...
 */
void FLuaAnimBlueprintEditorModule::HandleBlueprintPreCompile(
    UBlueprint* BlueprintToCompile)
{
    check(IsInGameThread());
    if (!IsValid(Cast<UAnimBlueprint>(BlueprintToCompile))) return;
    ULuaTransitionRuntimeLibrary::ResetRuntimeCachesForPIESession();
}
 ...
 */
void FLuaAnimBlueprintEditorModule::HandlePreBeginPIE(
 ...
 */
void FLuaAnimBlueprintEditorModule::HandleEndPIE(
    const bool bIsSimulatingInEditor)
{
    static_cast<void>(bIsSimulatingInEditor);
    ULuaTransitionRuntimeLibrary::ResetRuntimeCachesForPIESession();
 ...
 */
void FLuaAnimBlueprintEditorModule::MarkPendingSourceChanges(
    const TWeakPtr<FLuaAnimBlueprintAutoCompileScheduler, ESPMode::ThreadSafe> WeakScheduler)
{
    const TSharedPtr<FLuaAnimBlueprintAutoCompileScheduler, ESPMode::ThreadSafe> PinnedScheduler =
        WeakScheduler.Pin();
...
IMPLEMENT_MODULE(FLuaAnimBlueprintEditorModule, LuaAnimBlueprintEditor)
