...
namespace LuaBehaviorTreeIR
{
    ...
    struct FParseContext
    {
        ...
        TArray<FLuaBehaviorTreeDiagnostic>* Diagnostics = nullptr;   // 诊断接收数组
    };
    ...
    void AddError(
        ...
        const FString& Path,
        const FLuaBehaviorTreeSourceLocation& Location)
    {
        FLuaBehaviorTreeDiagnostic& Diagnostic = Context.Diagnostics->AddDefaulted_GetRef();
    ...
    /**
     * 从 Lua table 读取 UnLua 暴露的原生 UENUM integer，并按期望枚举类型校验值域。
     * 本函数不接受成员名称字符串，结束时恢复进入前的 Lua 栈顶。
     *
     * @param Context 当前解析上下文。
     * @param TableIndex Lua table 栈索引。
     * @param FieldName 字段名。
     ...
     * @return 字段为合法原生枚举 integer 时返回 true。
     */
        ...
        EnumType& OutValue)
    {
        ...
        if (lua_getfield(Context.State, lua_absindex(Context.State, TableIndex), FieldName) != LUA_TNUMBER
            || !lua_isinteger(Context.State, -1))
        {
            AddError(Context, InvalidLuaType, TEXT("字段必须是原生 UENUM integer。"), Path, Location);
            return false;
        }
        ...
        if (!Enum || !Enum->IsValidEnumValue(RawValue))
        {
            AddError(
                Context,
                InvalidValueType,
                FString::Printf(TEXT("枚举数值 %lld 不属于期望的 UENUM。"), RawValue),
                Path,
                Location);
            return false;
        }
    ...
    bool ParseLocation(
        FParseContext& Context,
        const int32 TableIndex,
        FLuaBehaviorTreeSourceLocation& OutLocation)
    {
        ...
        int32& OutIndex)
    {
        ...
        if (!ReadEnum(Context, TableIndex, "Type", Path + TEXT(".Type"), Location, Value.Type)) return false;

        OutIndex = OutIR.Values.Add(Value);
        FLuaBehaviorTreeIRValue& StoredValue = OutIR.Values[OutIndex];
        const int32 InitialTop = lua_gettop(Context.State);
        ON_SCOPE_EXIT { lua_settop(Context.State, InitialTop); };
        const int32 RawType = lua_getfield(Context.State, lua_absindex(Context.State, TableIndex), "Value");

        if (StoredValue.Type == ELuaBehaviorTreeValueType::Bool)
        {
        ...
        if (StoredValue.Type == ELuaBehaviorTreeValueType::Integer)
        {
        ...
        if (StoredValue.Type == ELuaBehaviorTreeValueType::Float)
        {
        ...
        if (StoredValue.Type == ELuaBehaviorTreeValueType::Enum)
        {
            if (RawType != LUA_TNUMBER || !lua_isinteger(Context.State, -1))
            {
                AddError(Context, InvalidLuaType, TEXT("Enum.Value 必须是原生 UENUM integer。"), Path, Location);
                return false;
            }
            StoredValue.IntegerValue = static_cast<int64>(lua_tointeger(Context.State, -1));
            return true;
        }
        if (StoredValue.Type != ELuaBehaviorTreeValueType::Struct
            && StoredValue.Type != ELuaBehaviorTreeValueType::Array)
        {
        ...
        if (StoredValue.Type == ELuaBehaviorTreeValueType::Array)
        {
        ...
        TArray<FLuaBehaviorTreeIRProperty>& OutProperties)
    {
        ...
        for (int32 PropertyIndex = 1; PropertyIndex <= Count; ++PropertyIndex)
        {
            ...
            FLuaBehaviorTreeIRProperty& Property = OutProperties.AddDefaulted_GetRef();
    ...
    bool ParseNodes(
        ...
        const char* FieldName,
        FLuaBehaviorTreeIR& OutIR,
        TArray<FLuaBehaviorTreeIRNode>& OutNodes)
    {
        const int32 InitialTop = lua_gettop(Context.State);
        ON_SCOPE_EXIT { lua_settop(Context.State, InitialTop); };
        if (lua_getfield(Context.State, lua_absindex(Context.State, RootTableIndex), FieldName) != LUA_TTABLE)
        {
            AddError(Context, InvalidLuaType, TEXT("节点集合必须是数组 table。"), UTF8_TO_TCHAR(FieldName), FLuaBehaviorTreeSourceLocation());
            return false;
        }
        ...
        for (int32 NodeIndex = 1; NodeIndex <= Count; ++NodeIndex)
        {
            ...
            if (lua_type(Context.State, -1) != LUA_TTABLE)
            {
                AddError(Context, InvalidLuaType, TEXT("节点数组元素必须是 table。"), UTF8_TO_TCHAR(FieldName), FLuaBehaviorTreeSourceLocation());
                return false;
            }
            const int32 NodeTableIndex = lua_absindex(Context.State, -1);
            FLuaBehaviorTreeIRNode& Node = OutNodes.AddDefaulted_GetRef();
        ...
        FLuaBehaviorTreeIR& OutIR)
    {
        ...
        {
            AddError(Context, InvalidLuaType, TEXT("BlackboardKeys 必须是数组 table。"), TEXT("BlackboardKeys"), FLuaBehaviorTreeSourceLocation());
            return false;
        }
        const int32 ArrayIndex = lua_absindex(Context.State, -1);
        const int32 Count = static_cast<int32>(lua_rawlen(Context.State, ArrayIndex));
        for (int32 KeyIndex = 1; KeyIndex <= Count; ++KeyIndex)
        {
            ...
            if (lua_type(Context.State, -1) != LUA_TTABLE)
            {
                AddError(Context, InvalidLuaType, TEXT("BlackboardKeys 元素必须是 table。"), TEXT("BlackboardKeys"), FLuaBehaviorTreeSourceLocation());
                return false;
            }
     ...
     */
    bool ParseIR(FParseContext& Context, const int32 TableIndex, FLuaBehaviorTreeIR& OutIR)
    {
        FString ParentBlackboard;
        return ReadInteger(Context, TableIndex, "SchemaVersion", TEXT("IR.SchemaVersion"), FLuaBehaviorTreeSourceLocation(), OutIR.SchemaVersion)
            && ReadString(Context, TableIndex, "SourceModule", TEXT("IR.SourceModule"), FLuaBehaviorTreeSourceLocation(), OutIR.SourceModule)
            && ReadString(Context, TableIndex, "RootNodeId", TEXT("IR.RootNodeId"), FLuaBehaviorTreeSourceLocation(), OutIR.RootNodeId)
            && ReadString(Context, TableIndex, "ParentBlackboard", TEXT("IR.ParentBlackboard"), FLuaBehaviorTreeSourceLocation(), ParentBlackboard)
        ...
        TArray<FLuaBehaviorTreeDiagnostic>& OutDiagnostics)
    {
        ...
        FLuaBehaviorTreeSourceLocation Location;
 ...
 */
bool ULuaBehaviorTreeIRLibrary::CompileLuaModule(
    const FString& LuaModuleName,
    FLuaBehaviorTreeIR& OutIR,
    TArray<FLuaBehaviorTreeDiagnostic>& OutDiagnostics)
{
    using namespace LuaBehaviorTreeIR;
 ...
 */
bool ULuaBehaviorTreeIRLibrary::CompileLuaModuleFresh(
    const FString& LuaModuleName,
    FLuaBehaviorTreeIR& OutIR,
    TArray<FLuaBehaviorTreeDiagnostic>& OutDiagnostics)
{
    OutIR = FLuaBehaviorTreeIR();
    OutDiagnostics.Reset();
    if (!LuaBehaviorTreeIR::InvalidateLuaModuleCache(
            LuaModuleName,
            OutDiagnostics))
    {
        return false;
 ...
 */
bool ULuaBehaviorTreeIRLibrary::Validate(
    const FLuaBehaviorTreeIR& IR,
    TArray<FLuaBehaviorTreeDiagnostic>& OutDiagnostics)
{
    using namespace LuaBehaviorTreeIR;
    ...
    bool bValid = true;
    TSet<FString> AllIds;
    TMap<FString, const FLuaBehaviorTreeIRNode*> MainNodes;
    TMap<FString, FString> ParentById;
    if (IR.SchemaVersion != 1)
    {
        AddValidationError(OutDiagnostics, InvalidInput, TEXT("当前仅支持 SchemaVersion=1。"), TEXT("SchemaVersion"), FLuaBehaviorTreeSourceLocation());
        bValid = false;
    }
    if (!IR.ParentBlackboard.IsNull() && !Cast<UBlackboardData>(IR.ParentBlackboard.TryLoad()))
    {
        AddValidationError(OutDiagnostics, InvalidClass, TEXT("ParentBlackboard 无法加载为 UBlackboardData。"), IR.ParentBlackboard.ToString(), FLuaBehaviorTreeSourceLocation());
        bValid = false;
    }
    ...
    const FLuaBehaviorTreeIRNode* const* RootNode = MainNodes.Find(IR.RootNodeId);
    ...
    {
        AddValidationError(OutDiagnostics, MissingRoot, TEXT("RootNodeId 必须指向 Composite 主节点。"), IR.RootNodeId, FLuaBehaviorTreeSourceLocation());
        bValid = false;
    }
    ...
    for (const FLuaBehaviorTreeIRNode& Node : IR.Nodes)
    {
        ...
        const FLuaBehaviorTreeIRNode* const* ParentNode = MainNodes.Find(Node.ParentId);
    ...
    for (const TPair<FString, FString>& Pair : ParentById)
    {
        ...
        {
            AddValidationError(OutDiagnostics, GraphCycle, TEXT("行为树父链存在环。"), Pair.Key, FLuaBehaviorTreeSourceLocation());
            bValid = false;
            break;
        }
    }
    ...
    for (int32 ValueIndex = 0; ValueIndex < IR.Values.Num(); ++ValueIndex)
    {
        ...
        for (const TPair<FName, int32>& FieldPair : Value.StructFields)
        {
            ...
            {
                AddValidationError(OutDiagnostics, InvalidInput, TEXT("Struct 字段引用了无效值池索引。"), FString::FromInt(ValueIndex), FLuaBehaviorTreeSourceLocation());
                bValid = false;
            }
        }
        for (const int32 ChildIndex : Value.ArrayItems)
        {
            ...
            {
                AddValidationError(OutDiagnostics, InvalidInput, TEXT("Array 元素引用了无效值池索引。"), FString::FromInt(ValueIndex), FLuaBehaviorTreeSourceLocation());
                bValid = false;
            }
        }
    }
