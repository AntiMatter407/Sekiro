...
namespace LuaBehaviorTreeExporter
{
    ...
    {
        FLuaBehaviorTreeIR* IR = nullptr; // 正在构建的规范 IR
        TArray<FLuaBehaviorTreeDiagnostic>* Diagnostics = nullptr; // 诊断输出
        ...
        TArray<FLuaBehaviorTreeDiagnostic>& Diagnostics)
    {
        ...
        FLuaBehaviorTreeIRValue Value;

        if (const FBoolProperty* BoolProperty = CastField<FBoolProperty>(Property))
        {
            Value.Type = ELuaBehaviorTreeValueType::Bool;
            Value.BoolValue = BoolProperty->GetPropertyValue(ValueAddress);
        }
        else if (const FEnumProperty* EnumProperty = CastField<FEnumProperty>(Property))
        {
            Value.Type = ELuaBehaviorTreeValueType::Enum;
            Value.IntegerValue = EnumProperty->GetUnderlyingProperty()
                ->GetSignedIntPropertyValue(ValueAddress);
            Value.EnumTypeName = EnumProperty->GetEnum()->GetFName();
        }
        else if (const FByteProperty* ByteProperty = CastField<FByteProperty>(Property))
        {
            if (ByteProperty->Enum)
            {
                ...
                Value.EnumTypeName = ByteProperty->Enum->GetFName();
            }
            else
            {
                Value.Type = ELuaBehaviorTreeValueType::Integer;
            ...
            CastField<FNumericProperty>(Property))
        {
            ...
            {
                Value.Type = ELuaBehaviorTreeValueType::Float;
                Value.FloatValue =
        ...
        {
            Value.Type = ELuaBehaviorTreeValueType::SoftClass;
        ...
        {
            Value.Type = ELuaBehaviorTreeValueType::SoftObject;
        ...
        {
            Value.Type = ELuaBehaviorTreeValueType::Class;
        ...
        {
            Value.Type = ELuaBehaviorTreeValueType::Object;
        ...
        {
            Value.Type = ELuaBehaviorTreeValueType::Struct;
        ...
        {
            Value.Type = ELuaBehaviorTreeValueType::Array;
            OutValueIndex = IR.Values.Add(Value);
    ...
    bool ExtractProperties(
        UObject* Object,
        FLuaBehaviorTreeIR& IR,
        TArray<FLuaBehaviorTreeIRProperty>& OutProperties,
        TArray<FLuaBehaviorTreeDiagnostic>& Diagnostics)
    {
        ...
        for (int32 PropertyIndex = 0; PropertyIndex < Properties.Num(); ++PropertyIndex)
        {
            ...
            FLuaBehaviorTreeIRProperty& IRProperty = OutProperties.AddDefaulted_GetRef();
        ...
        for (const UAIGraphNode* Parent : Context.OrderedNodes)
        {
            ...
            for (const UAIGraphNode* SubNode : Parent->SubNodes)
            {
                ...
                if (UBTDecorator* DecoratorInstance = Cast<UBTDecorator>(SubNode->NodeInstance))
                {
                    ...
                    FLuaBehaviorTreeIRNode& Node = Context.IR->Decorators.AddDefaulted_GetRef();
                ...
                if (UBTService* ServiceInstance = Cast<UBTService>(SubNode->NodeInstance))
                {
                    ...
                    FLuaBehaviorTreeIRNode& Node = Context.IR->Services.AddDefaulted_GetRef();
        ...
        const int32 Indent)
    {
        ...
        const FLuaBehaviorTreeIRValue& Value = IR.Values[ValueIndex];
        ...
        if (Value.Type == ELuaBehaviorTreeValueType::Name) TypeName = TEXT("Name");
        else if (Value.Type == ELuaBehaviorTreeValueType::Text) TypeName = TEXT("Text");
        else if (Value.Type == ELuaBehaviorTreeValueType::Enum)
        {
            const UEnum* Enum = Value.EnumTypeName.IsNone()
                ? nullptr
                : FindFirstObject<UEnum>(*Value.EnumTypeName.ToString());
            FString MemberName = Enum
                ? Enum->GetNameStringByValue(Value.IntegerValue)
                : FString();
            if (Enum && !MemberName.IsEmpty())
            {
                MemberName.Split(
                    TEXT("::"),
                    nullptr,
                ...
                return FString::Printf(
                    TEXT("LuaBehaviorTree.Value.Enum(UE.%s.%s)"),
                    *Enum->GetName(),
                    *MemberName);
            }
            return FString::Printf(TEXT("LuaBehaviorTree.Value.Enum(%lld)"), Value.IntegerValue);
        }
        else if (Value.Type == ELuaBehaviorTreeValueType::Object) TypeName = TEXT("Object");
        else if (Value.Type == ELuaBehaviorTreeValueType::SoftObject) TypeName = TEXT("SoftObject");
        else if (Value.Type == ELuaBehaviorTreeValueType::Class) TypeName = TEXT("Class");
        else if (Value.Type == ELuaBehaviorTreeValueType::SoftClass) TypeName = TEXT("SoftClass");
        ...
        FString Result = Value.Type == ELuaBehaviorTreeValueType::Struct
        ...
        const int32 Indent)
    {
        ...
        TArray<const FLuaBehaviorTreeIRProperty*> SortedProperties;
        for (const FLuaBehaviorTreeIRProperty& Property : Properties)
            SortedProperties.Add(&Property);
        SortedProperties.Sort([](
            const FLuaBehaviorTreeIRProperty& Left,
            const FLuaBehaviorTreeIRProperty& Right)
        {
        ...
        const FString ChildIndent = FString::ChrN(Indent + 4, TEXT(' '));
        FString Result(TEXT("{\r\n"));
        for (const FLuaBehaviorTreeIRProperty* Property : SortedProperties)
        {
            Result += ChildIndent
    ...
    FString RenderLuaSource(const FLuaBehaviorTreeIR& IR)
    {
        FString Source;
    ...
    bool AreValuesEquivalent(
        const FLuaBehaviorTreeIR& LeftIR,
        const int32 LeftIndex,
        const FLuaBehaviorTreeIR& RightIR,
        const int32 RightIndex)
    {
        if (!LeftIR.Values.IsValidIndex(LeftIndex) || !RightIR.Values.IsValidIndex(RightIndex)) return false;
        const FLuaBehaviorTreeIRValue& Left = LeftIR.Values[LeftIndex];
        const FLuaBehaviorTreeIRValue& Right = RightIR.Values[RightIndex];
    ...
    bool ArePropertiesEquivalent(
        const FLuaBehaviorTreeIR& LeftIR,
        const TArray<FLuaBehaviorTreeIRProperty>& Left,
        const FLuaBehaviorTreeIR& RightIR,
        const TArray<FLuaBehaviorTreeIRProperty>& Right)
    {
    ...
    bool AreIRsEquivalent(
        const FLuaBehaviorTreeIR& Left,
        const FLuaBehaviorTreeIR& Right)
    {
        ...
        {
            const FLuaBehaviorTreeIRNode& A = Left.Nodes[Index];
            const FLuaBehaviorTreeIRNode& B = Right.Nodes[Index];
        ...
        {
            const FLuaBehaviorTreeIRNode& A = Left.Decorators[Index];
            const FLuaBehaviorTreeIRNode& B = Right.Decorators[Index];
            if (A.Id != B.Id || A.ParentId != B.ParentId || A.ClassPath != B.ClassPath
                || !ArePropertiesEquivalent(Left, A.Properties, Right, B.Properties)) return false;
        }
        for (int32 Index = 0; Index < Left.Services.Num(); ++Index)
        {
            const FLuaBehaviorTreeIRNode& A = Left.Services[Index];
            const FLuaBehaviorTreeIRNode& B = Right.Services[Index];
            if (A.Id != B.Id || A.ParentId != B.ParentId || A.ClassPath != B.ClassPath
                || !ArePropertiesEquivalent(Left, A.Properties, Right, B.Properties)) return false;
        }
        for (int32 Index = 0; Index < Left.BlackboardKeys.Num(); ++Index)
        {
            const FLuaBlackboardIRKey& A = Left.BlackboardKeys[Index];
            const FLuaBlackboardIRKey& B = Right.BlackboardKeys[Index];
 ...
 */
bool ULuaBehaviorTreeExporterLibrary::ExtractBehaviorTreeIR(
    UBehaviorTree* BehaviorTree,
    const FString& SourceModule,
    FLuaBehaviorTreeIR& OutIR,
    TArray<FLuaBehaviorTreeDiagnostic>& OutDiagnostics)
{
    using namespace LuaBehaviorTreeExporter;
    OutIR = FLuaBehaviorTreeIR();
    ...
    if (Blackboard)
    {
        ...
        for (const FBlackboardEntry& Entry : Blackboard->Keys)
        {
            ...
            FLuaBlackboardIRKey& Key = OutIR.BlackboardKeys.AddDefaulted_GetRef();
    ...
    return ULuaBehaviorTreeIRLibrary::Validate(OutIR, OutDiagnostics);
}
 ...
 */
bool ULuaBehaviorTreeExporterLibrary::ExportBehaviorTreeToLua(
    UBehaviorTree* BehaviorTree,
    ...
    TArray<FLuaBehaviorTreeDiagnostic>& OutDiagnostics)
{
    ...
    TArray<FLuaBehaviorTreeDiagnostic> RoundTripDiagnostics;
    const bool bRoundTripCompiled = ULuaBehaviorTreeIRLibrary::CompileLuaModuleFresh(
        LuaModuleName,
        RoundTripIR,
    ...
    if (!bRoundTripCompiled || !AreIRsEquivalent(ExportedIR, RoundTripIR))
    {
        ...
        FLuaBehaviorTreeIR RestoredIR;
        TArray<FLuaBehaviorTreeDiagnostic> RestoreDiagnostics;
    ...
    ULuaBehaviorTreeFactoryLibrary::GetLuaAssetConfiguration(BehaviorTree, Configuration);
    Configuration.LuaModuleName = LuaModuleName;
    Configuration.SourceMode = ELuaBehaviorTreeSourceMode::BehaviorTree;
    ULuaBehaviorTreeFactoryLibrary::SetLuaAssetConfiguration(BehaviorTree, Configuration);
    return true;
}
 ...
 */
bool ULuaBehaviorTreeExporterLibrary::ResolveLuaModuleFilePath(
    const FString& LuaModuleName,
    FString& OutFilePath,
    TArray<FLuaBehaviorTreeDiagnostic>& OutDiagnostics)
{
