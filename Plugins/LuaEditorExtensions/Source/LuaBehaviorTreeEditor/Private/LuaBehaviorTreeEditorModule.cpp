#include "LuaBehaviorTreeEditorModule.h"
...
#define LOCTEXT_NAMESPACE "LuaBehaviorTreeEditor"

namespace
{
    const FName BehaviorTreeToolbarName(
        TEXT("AssetEditor.Behavior Tree.ToolBar.BehaviorTree"));
    const FName MessageLogName(TEXT("LuaBehaviorTree"));
        ...
        const TArray<FLuaBehaviorTreeDiagnostic>& Diagnostics)
    {
        ...
        for (const FLuaBehaviorTreeDiagnostic& Diagnostic : Diagnostics)
        {
            const FText Text = FText::FromString(FString::Printf(
                ...
                *Diagnostic.Message,
                *Diagnostic.Path));
            if (Diagnostic.Severity == ELuaBehaviorTreeDiagnosticSeverity::Error)
                MessageLog.Error(Text);
            else if (Diagnostic.Severity == ELuaBehaviorTreeDiagnosticSeverity::Warning)
    ...
    bool CanRunAssetAction(const TWeakObjectPtr<UBehaviorTree> BehaviorTree)
    {
        ...
        return ULuaBehaviorTreeFactoryLibrary::GetLuaAssetConfiguration(
                BehaviorTree.Get(),
                Configuration)
    ...
    void BuildBehaviorTreeToolbar(UToolMenu* Menu)
    {
        ...
        // Lua 模块未配置或处于 PIE/SIE 时，不显示无论如何都不能执行的同步按钮。
        if (!CanRunAssetAction(WeakBehaviorTree)) return;
 ...
 */
void FLuaBehaviorTreeEditorModule::StartupModule()
{
    ...
    UToolMenus::RegisterStartupCallback(
        FSimpleMulticastDelegate::FDelegate::CreateRaw(
            this,
            &FLuaBehaviorTreeEditorModule::RegisterMenus));
}
 ...
 */
void FLuaBehaviorTreeEditorModule::ShutdownModule()
{
 ...
 */
void FLuaBehaviorTreeEditorModule::RegisterMenus()
{
...
IMPLEMENT_MODULE(
    FLuaBehaviorTreeEditorModule,
    LuaBehaviorTreeEditor)
