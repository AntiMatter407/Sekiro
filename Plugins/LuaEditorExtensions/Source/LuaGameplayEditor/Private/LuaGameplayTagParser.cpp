...
namespace LuaGameplayTagParsing
{
    ...
    public:
        ...
        bool Run(TArray<FLuaGameplayTagEntry>& OutTags, FString& OutError);
    ...
    private:
        ...
        TArray<FLuaGameplayTagEntry> Tags; // 完整成功前不发布的临时结果
    };
    ...
    bool FParser::Run(TArray<FLuaGameplayTagEntry>& OutTags, FString& OutError)
    {
        OutTags.Reset();
        OutError.Reset();
        bool bValid = Next();
        ...
        Tags.Sort([](const FLuaGameplayTagEntry& Left, const FLuaGameplayTagEntry& Right)
        {
 ...
 */
bool FLuaGameplayTagParser::Parse(const FString& Source, const FString& SourceName, TArray<FLuaGameplayTagEntry>& OutTags, FString& OutError)
{
    if (FTCHARToUTF8(*Source, Source.Len()).Length() > LuaGameplayTagParsing::MaxSourceBytes)
    {
        OutTags.Reset();
        OutError = SourceName + TEXT(":1:1: Lua 输入超过 1 MiB");
        return false;
    }
    LuaGameplayTagParsing::FParser Parser(Source, SourceName);
 ...
 */
bool FLuaGameplayTagParser::ValidateAssetPackagePath(const FString& PackagePath, FString& OutError)
{
    OutError.Reset();
    FText Reason;
