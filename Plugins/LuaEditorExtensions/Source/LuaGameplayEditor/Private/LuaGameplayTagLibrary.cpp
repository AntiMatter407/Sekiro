...
namespace LuaGameplayTagAssets
{
    const TCHAR* GeneratorKey = TEXT("LuaGameplay.Generator");
    const TCHAR* GeneratorValue = TEXT("LuaGameplayTags.v1");
    const TCHAR* SourceKey = TEXT("LuaGameplay.LuaSource");
 ...
 */
bool ULuaGameplayTagLibrary::PreviewGameplayTags(const FString& LuaFilePath, TArray<FLuaGameplayTagEntry>& OutTags, FString& OutError)
{
    OutTags.Reset();
    OutError.Reset();
    if (!LuaGameplayTagAssets::CheckEditorContext(OutError)) return false;
    const FString Filename = LuaGameplayTagAssets::NormalizeSource(LuaFilePath);
    ...
    if (Bytes.Num() > 1024 * 1024 || !LuaGameplayTagAssets::IsValidUtf8(Bytes))
    {
    ...
    return FLuaGameplayTagParser::Parse(Source, Filename, OutTags, OutError);
}
 ...
 */
bool ULuaGameplayTagLibrary::GenerateGameplayTagTable(const FString& LuaFilePath, const FString& AssetPackagePath, bool bRegisterTagSource, FLuaGameplayTagGenerationResult& OutResult)
{
    OutResult = FLuaGameplayTagGenerationResult();
    TArray<FLuaGameplayTagEntry> Tags;
    if (!PreviewGameplayTags(LuaFilePath, Tags, OutResult.Message)
        || !FLuaGameplayTagParser::ValidateAssetPackagePath(AssetPackagePath, OutResult.Message)) return false;
    OutResult.TagCount = Tags.Num();
    const FString SourcePath = LuaGameplayTagAssets::NormalizeSource(LuaFilePath);
    ...
    if (!LuaGameplayTagAssets::CheckWritable(Filename, OutResult.Message)) return false;
    ...
    if (Table)
    {
        ...
        FMetaData* Metadata = &Table->GetOutermost()->GetMetaData();
#else
        UMetaData* Metadata = Table->GetOutermost()->GetMetaData();
#endif
        if (Table->GetRowStruct() != FGameplayTagTableRow::StaticStruct()
            || Metadata->GetValue(Table, LuaGameplayTagAssets::GeneratorKey) != LuaGameplayTagAssets::GeneratorValue
            || !FPaths::IsSamePath(LuaGameplayTagAssets::NormalizeSource(Metadata->GetValue(Table, LuaGameplayTagAssets::SourceKey)), SourcePath))
        {
    ...
    Table->EmptyTable();
    for (const FLuaGameplayTagEntry& Entry : Tags)
    {
...
#endif
    const FString PreviousGenerator = Metadata->GetValue(Table, LuaGameplayTagAssets::GeneratorKey);
    const FString PreviousSource = Metadata->GetValue(Table, LuaGameplayTagAssets::SourceKey);
    Metadata->SetValue(Table, LuaGameplayTagAssets::GeneratorKey, LuaGameplayTagAssets::GeneratorValue);
    Metadata->SetValue(Table, LuaGameplayTagAssets::SourceKey, *LuaGameplayTagAssets::MakeSourceIdentity(SourcePath));
    ...
    if (!UPackage::SavePackage(Package, Table, *Filename, SaveArgs))
    {
        ...
        if (bNewAsset)
        {
            Metadata->RemoveValue(Table, LuaGameplayTagAssets::GeneratorKey);
            Metadata->RemoveValue(Table, LuaGameplayTagAssets::SourceKey);
            ...
            Package->Rename(*FString::Printf(TEXT("/Temp/LuaGameplayFailed_%s"), *FGuid::NewGuid().ToString(EGuidFormats::Digits)), nullptr, REN_DontCreateRedirectors | REN_NonTransactional);
        }
        else
        {
            Metadata->SetValue(Table, LuaGameplayTagAssets::GeneratorKey, *PreviousGenerator);
            Metadata->SetValue(Table, LuaGameplayTagAssets::SourceKey, *PreviousSource);
        }
    ...
    {
        if (!LuaGameplayTagAssets::RegisterTableSource(FSoftObjectPath(Table), OutResult.Message))
        {
