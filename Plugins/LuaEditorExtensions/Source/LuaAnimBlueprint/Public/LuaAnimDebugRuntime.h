...
struct LUAANIMBLUEPRINT_API FLuaAnimTransitionDebugValue
{
...
struct LUAANIMBLUEPRINT_API FLuaAnimDebugNode
{
    ...
    TArray<FLuaAnimDebugNode> Children; // 所有 relevant 子贡献
};

/** 实时 UI 与 JSONL 共用的不可变帧快照。 */
struct LUAANIMBLUEPRINT_API FLuaAnimDebugFrame
{
    ...
    TArray<FLuaAnimDebugNode> Roots;         // 完整 relevant 层级
    TArray<FLuaAnimTransitionDebugValue> Transitions; // 最近 Transition 求值
};

/** Lua 动画层级屏幕调试与 JSONL 快照记录器。 */
class LUAANIMBLUEPRINT_API FLuaAnimDebugRuntime
{
public:
    static void Startup();
    ...
    static void RecordTransitionValue(
        ...
        bool bRuleResult,
        bool bIsFinal);
    static bool GetLatestFrame(FLuaAnimDebugFrame& OutFrame);
    ...
    static FLuaAnimDebugNode ParseDebugLineForTesting(const FString& DebugLine);
    static FString BuildRealtimeTextForTesting(const FLuaAnimDebugFrame& Frame);
