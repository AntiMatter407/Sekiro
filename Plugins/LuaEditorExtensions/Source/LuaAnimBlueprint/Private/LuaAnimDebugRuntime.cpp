...
namespace LuaAnimDebugPrivate
{
    ...
    struct FPendingTransitionDebugValue
    {
        ...
        FLuaAnimTransitionDebugValue Value; // 工作线程完成构造的不可变调试值
    };
    ...
    struct FRuntimeState
    {
        ...
        TMap<TWeakObjectPtr<UAnimInstance>, TArray<FLuaAnimTransitionDebugValue>> Transitions; // 最近规则值
        FLuaAnimDebugFrame LatestFrame; // 供 Editor 读取的最近帧
    ...
    void StoreLatestTransitionValue(
        TArray<FLuaAnimTransitionDebugValue>& Values,
        FLuaAnimTransitionDebugValue&& Value)
    {
        FLuaAnimTransitionDebugValue* Existing = Values.FindByPredicate(
            [&Value](const FLuaAnimTransitionDebugValue& Candidate)
            {
    ...
    void ParseStateMachine(FLuaAnimDebugNode& Node)
    {
     ...
     */
    void ParseSlotInputs(FLuaAnimDebugNode& Node)
    {
    ...
    void BuildFullDebugTree(UAnimInstance* AnimInstance, const FString& LuaModuleName, TArray<FLuaAnimDebugNode>& OutRoots)
    {
        OutRoots.Reset();
        ...
        TFunction<FLuaAnimDebugNode(int32)> BuildNode = [&](const int32 Index)
        {
            FLuaAnimDebugNode Node = Nodes[Index].Node;
            ...
            if (!Node.MachineName.IsEmpty())
            {
                ...
                for (const FLuaAnimDebugNode& Child : Node.Children)
                {
     ...
     */
    void CaptureFrameValues(UAnimInstance* AnimInstance, FLuaAnimDebugFrame& Frame)
    {
    ...
    void AppendStateSignature(const FLuaAnimDebugNode& Node, TArray<FString>& Parts)
    {
        Parts.Add(FString::Printf(
            ...
            *Node.CurrentState,
            *Node.NativeAssetName));
        for (const FLuaAnimDebugNode& Child : Node.Children) AppendStateSignature(Child, Parts);
    }

    /** 生成帧活跃拓扑签名；连续权重和播放时间不参与，避免每帧误判状态变化。 */
    FString BuildStateSignature(const FLuaAnimDebugFrame& Frame)
    {
        TArray<FString> Parts;
        for (const FLuaAnimDebugNode& Root : Frame.Roots) AppendStateSignature(Root, Parts);
        Parts.Sort();
     ...
     */
    FLuaAnimDebugFrame GatherFrame(
        ...
        UAnimInstance*& OutTarget)
    {
        ...
        const TArray<FLuaAnimTransitionDebugValue>* Values =
    ...
    TSharedPtr<FJsonObject> NodeToJson(const FLuaAnimDebugNode& Node)
    {
        ...
        for (const FLuaAnimDebugNode& Child : Node.Children) Children.Add(MakeShared<FJsonValueObject>(NodeToJson(Child)));
        Object->SetArrayField(TEXT("Children"), Children);
        return Object;
    }

    /** 将公共帧 Schema 序列化为一行压缩 JSON。 */
    FString FrameToJsonLine(const FLuaAnimDebugFrame& Frame)
    {
        TSharedPtr<FJsonObject> Object = MakeShared<FJsonObject>();
        ...
        for (const FLuaAnimDebugNode& Root : Frame.Roots) Roots.Add(MakeShared<FJsonValueObject>(NodeToJson(Root)));
        Object->SetArrayField(TEXT("Roots"), Roots);
        TArray<TSharedPtr<FJsonValue>> Transitions;
        for (const FLuaAnimTransitionDebugValue& Value : Frame.Transitions)
        {
    ...
    bool WriteFrame(const FLuaAnimDebugFrame& Frame)
    {
    ...
    void ConsumeCapturedTransitions(const FLuaAnimDebugFrame& Frame)
    {
        if (Frame.AnimInstancePath.IsEmpty()) return;
        for (TPair<TWeakObjectPtr<UAnimInstance>, TArray<FLuaAnimTransitionDebugValue>>& Pair : RuntimeState.Transitions)
        {
        ...
        const FString& OutputPath)
    {
        ...
        FLuaAnimDebugFrame Frame = GatherFrame(
            TEXT("Start"),
     ...
     */
    void AppendRealtimeHierarchy(const FLuaAnimDebugNode& Node, const int32 Indent, TArray<FString>& Lines)
    {
    ...
    FString BuildRealtimeText(const FLuaAnimDebugFrame& Frame)
    {
        ...
        Lines.Add(TEXT("------------------------------"));
        for (const FLuaAnimDebugNode& Root : Frame.Roots) AppendRealtimeHierarchy(Root, 0, Lines);
    ...
    bool Tick(const float DeltaSeconds)
    {
        ...
        if (RuntimeState.DebugView == EDebugView::Help)
        {
            ...
            if (GEngine != nullptr) GEngine->AddOnScreenDebugMessage(DebugMessageKey, 0.1f, FColor::Cyan,
                TEXT("Usage:\nLua.LuaAnim.Debug [Off]\nLua.LuaAnim.Snapshot [IntervalSeconds]\n  no interval: state changes only\nLua.LuaAnim.Snapshot.Stop"), false);
            return true;
        }
...
void FLuaAnimDebugRuntime::Startup()
{
    using namespace LuaAnimDebugPrivate;
    check(IsInGameThread());
    if (RuntimeState.DebugCommand != nullptr) return;
    RuntimeState.DebugCommand = IConsoleManager::Get().RegisterConsoleCommand(
        TEXT("Lua.LuaAnim.Debug"), TEXT("Lua.LuaAnim.Debug [Off]"),
    ...
    RuntimeState.SnapshotCommand = IConsoleManager::Get().RegisterConsoleCommand(
        TEXT("Lua.LuaAnim.Snapshot"),
        TEXT("Lua.LuaAnim.Snapshot [IntervalSeconds]; omit interval to capture state changes only."),
    ...
    RuntimeState.SnapshotStopCommand = IConsoleManager::Get().RegisterConsoleCommand(
        TEXT("Lua.LuaAnim.Snapshot.Stop"), TEXT("Stop and flush the active Lua animation snapshot session."),
...
void FLuaAnimDebugRuntime::Shutdown()
{
    using namespace LuaAnimDebugPrivate;
    check(IsInGameThread());
    StopSnapshot();
 ...
 */
bool FLuaAnimDebugRuntime::IsSamplingEnabled()
{
    using namespace LuaAnimDebugPrivate;
...
void FLuaAnimDebugRuntime::RecordAnimInstanceModule(UAnimInstance* AnimInstance, const FString& LuaModuleName)
{
    using namespace LuaAnimDebugPrivate;
 ...
 */
void FLuaAnimDebugRuntime::RecordTransitionValue(
    UAnimInstance* AnimInstance,
...
bool FLuaAnimDebugRuntime::GetLatestFrame(FLuaAnimDebugFrame& OutFrame)
{
    using namespace LuaAnimDebugPrivate;
    if (!RuntimeState.bHasLatestFrame)
    {
        OutFrame = FLuaAnimDebugFrame();
        return false;
    }
...
FString FLuaAnimDebugRuntime::GetSnapshotSessionPath()
{
    return LuaAnimDebugPrivate::RuntimeState.SnapshotPath;
}
 ...
 */
void FLuaAnimDebugRuntime::StopSnapshotSession()
{
    check(IsInGameThread());
    LuaAnimDebugPrivate::StopSnapshot();
}
 ...
 */
void FLuaAnimDebugRuntime::ResetForPIEStart()
{
    ResetForPIEEnd();
}
 ...
 */
void FLuaAnimDebugRuntime::ResetForPIEEnd()
{
    using namespace LuaAnimDebugPrivate;
    check(IsInGameThread());
    StopSnapshot();
    ...
    RuntimeState.LatestFrame = FLuaAnimDebugFrame();
...
void FLuaAnimDebugRuntime::ApplyDebugArgumentsForTesting(const TArray<FString>& Arguments)
{
    LuaAnimDebugPrivate::HandleDebugCommand(Arguments);
}

/** 测试专用：用显式临时路径开始替换式 Session。 */
bool FLuaAnimDebugRuntime::StartSnapshotForTesting(const float IntervalSeconds, const FString& OutputPath)
{
    return LuaAnimDebugPrivate::StartSnapshot(
        IntervalSeconds,
        true,
        OutputPath);
}
...
bool FLuaAnimDebugRuntime::ApplySnapshotArgumentsForTesting(
    const TArray<FString>& Arguments,
    const FString& OutputPath)
{
    return LuaAnimDebugPrivate::StartSnapshotFromArguments(
        Arguments,
        OutputPath);
}

/** 测试专用：安全停止并 Flush Session。 */
void FLuaAnimDebugRuntime::StopSnapshotForTesting()
{
    LuaAnimDebugPrivate::StopSnapshot();
}

/** 测试专用：查询 Hierarchy 是否为当前唯一实时页。 */
bool FLuaAnimDebugRuntime::IsDebugEnabledForTesting()
{
    return LuaAnimDebugPrivate::RuntimeState.DebugView == LuaAnimDebugPrivate::EDebugView::Hierarchy;
}

/** 测试专用：查询 Snapshot Session 是否活动。 */
bool FLuaAnimDebugRuntime::IsSnapshotActiveForTesting()
{
    return LuaAnimDebugPrivate::RuntimeState.bSnapshotActive;
}

/** 测试专用：查询当前 Session 是否启用额外定时采样。 */
bool FLuaAnimDebugRuntime::IsSnapshotIntervalSamplingEnabledForTesting()
{
    return LuaAnimDebugPrivate::RuntimeState.bIntervalSamplingEnabled;
}

/** 测试专用：读取经下限修正后的采样间隔秒数。 */
float FLuaAnimDebugRuntime::GetSnapshotIntervalForTesting()
{
    return LuaAnimDebugPrivate::RuntimeState.SnapshotInterval;
}

/** 测试专用：使用真实解析器把一条 UE 动画调试行转换为节点。 */
FLuaAnimDebugNode FLuaAnimDebugRuntime::ParseDebugLineForTesting(
    const FString& DebugLine)
{
    using namespace LuaAnimDebugPrivate;
    FLuaAnimDebugNode Node;
...
FString FLuaAnimDebugRuntime::BuildRealtimeTextForTesting(
    const FLuaAnimDebugFrame& Frame)
{
    return LuaAnimDebugPrivate::BuildRealtimeText(Frame);
}

/** 测试专用：恢复关闭状态并清空全部内存采样，保留命令注册。 */
void FLuaAnimDebugRuntime::ResetForTesting()
{
    using namespace LuaAnimDebugPrivate;
    ResetForPIEEnd();
