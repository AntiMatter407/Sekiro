# Lua Editor Extensions

`LuaEditorExtensions` 是项目无关的 Lua 编辑器扩展插件。它按能力拆分为互不依赖业务语义的模块：

- `LuaAnimBlueprint` / `LuaAnimBlueprintEditor`：Lua 动画蓝图图结构与编辑器编译能力。
- `LuaBehaviorTree` / `LuaBehaviorTreeEditor`：Lua 行为树与 Blackboard 编译、反向导出能力。
- `LuaGameplayEditor`：Lua Gameplay 数据编辑器工具；具体项目标签和资产路径仍由项目 Lua 提供。

插件不定义角色、状态枚举、项目资产路径或玩法 IR 字段。下面记录行为树模块的使用约定。
...
蓝图和 Python 也可以调用 `ULuaBehaviorTreeFactoryLibrary` 的
`GetLuaAssetConfiguration`、`SetLuaAssetConfiguration`、
`CheckConfiguredBehaviorTree` 和 `GenerateConfiguredBehaviorTree`。

`ULuaBehaviorTreeExporterLibrary::ExportBehaviorTreeToLua` 提供 Blueprint/Python 可调用的反向导出，
...
`ULuaBehaviorTreeTask` 是唯一的通用运行时 Lua Task。每个节点实例都可以设置不同的
