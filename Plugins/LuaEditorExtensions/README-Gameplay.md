...
1. 编译项目并启用 **LuaEditorExtensions**，新安装插件后重启编辑器。
...
模块 `LuaGameplayEditor` 的公共接口 `FLuaGameplayEditorModule::RegisterTool` 接收工具 ID、本地化的显示名称及 Slate Widget 工厂。
...
首个工具使用 `ULuaGameplayTagLibrary::PreviewGameplayTags` 和 `GenerateGameplayTagTable`，可供编辑器蓝图或自动化调用。
