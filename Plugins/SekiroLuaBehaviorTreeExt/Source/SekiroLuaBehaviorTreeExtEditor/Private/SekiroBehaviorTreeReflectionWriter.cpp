...
namespace
{
     ...
     *
     * @param Type 待判断标签。
     * @return String、Name、Text、Enum、对象路径或类路径标签返回 true。
     */
    bool IsStringBackedType(const ESekiroBehaviorTreeValueType Type)
    {
        return Type == ESekiroBehaviorTreeValueType::String
            || Type == ESekiroBehaviorTreeValueType::Name
            || Type == ESekiroBehaviorTreeValueType::Text
            || Type == ESekiroBehaviorTreeValueType::Enum
            || Type == ESekiroBehaviorTreeValueType::Object
            || Type == ESekiroBehaviorTreeValueType::SoftObject
            || Type == ESekiroBehaviorTreeValueType::Class
            || Type == ESekiroBehaviorTreeValueType::SoftClass;
    }
}
 ...
 */
bool FSekiroBehaviorTreeReflectionWriter::ApplyProperties(
    UObject* Target,
    ...
    TArray<FSekiroBehaviorTreeDiagnostic>& OutDiagnostics)
{
    ...
    bool bSuccess = true;
    for (const FSekiroBehaviorTreeIRProperty& IRProperty : Properties)
    {
 ...
 */
bool FSekiroBehaviorTreeReflectionWriter::WriteValue(
    FProperty* Property,
    void* ValueAddress,
    ...
    TArray<FSekiroBehaviorTreeDiagnostic>& OutDiagnostics)
{
    ...
    {
        if (Value.Type != ESekiroBehaviorTreeValueType::Enum)
        {
            AddError(PropertyTypeMismatch, TEXT("Enum 属性要求 Value.Enum。"), Path, SourceLocation, OutDiagnostics);
            return false;
        }
        const int64 EnumValue = EnumProperty->GetEnum()->GetValueByNameString(Value.StringValue);
        if (EnumValue == INDEX_NONE)
        {
            AddError(InvalidEnumValue, TEXT("枚举成员名称无效。"), Path, SourceLocation, OutDiagnostics);
            return false;
        }
    ...
    if (FByteProperty* ByteProperty = CastField<FByteProperty>(Property))
    {
        if (ByteProperty->Enum)
        {
            ...
            const int64 EnumValue = ByteProperty->Enum->GetValueByNameString(Value.StringValue);
            if (EnumValue == INDEX_NONE || EnumValue > MAX_uint8)
            {
                AddError(InvalidEnumValue, TEXT("枚举成员名称无效或超出 uint8。"), Path, SourceLocation, OutDiagnostics);
                return false;
            }
    ...
    {
        if (Value.Type != ESekiroBehaviorTreeValueType::String)
        {
    ...
    {
        if (Value.Type != ESekiroBehaviorTreeValueType::Name)
        {
    ...
    {
        if (Value.Type != ESekiroBehaviorTreeValueType::Text)
        {
    ...
    {
        if (Value.Type != ESekiroBehaviorTreeValueType::SoftClass)
        {
    ...
    {
        if (Value.Type != ESekiroBehaviorTreeValueType::SoftObject)
        {
    ...
    {
        if (Value.Type != ESekiroBehaviorTreeValueType::Class)
        {
    ...
    {
        if (Value.Type != ESekiroBehaviorTreeValueType::Object)
        {
    ...
    {
        if (Value.Type != ESekiroBehaviorTreeValueType::Array)
        {
        ...
        for (int32 ItemIndex = 0; ItemIndex < Value.ArrayItems.Num(); ++ItemIndex)
        {
            ...
            if (!Values.IsValidIndex(ValueIndex)
                || !WriteValue(ArrayProperty->Inner, ArrayHelper.GetRawPtr(ItemIndex), Values.IsValidIndex(ValueIndex) ? Values[ValueIndex] : FSekiroBehaviorTreeIRValue(), Values,
 ...
 */
void FSekiroBehaviorTreeReflectionWriter::AddError(
    const FName Code,
    ...
    TArray<FSekiroBehaviorTreeDiagnostic>& OutDiagnostics)
{
    FSekiroBehaviorTreeDiagnostic& Diagnostic = OutDiagnostics.AddDefaulted_GetRef();
