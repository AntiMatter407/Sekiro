...
namespace SekiroAnimGraphIRLuaWriterPrivate
{
    /**
        ...
        FString& OutText)
    {
        ...
        if (const FEnumProperty* EnumProperty = CastField<FEnumProperty>(&Property))
        {
            ...
            FString EnumName = EnumProperty->GetEnum()->GetNameStringByValue(RawValue);
            EnumName.Split(TEXT("::"), nullptr, &EnumName, ESearchCase::CaseSensitive, ESearchDir::FromEnd);
            OutText += Quote(EnumName);
            return true;
        }
        if (const FByteProperty* ByteProperty = CastField<FByteProperty>(&Property))
        {
            ...
            {
                OutText += Quote(ByteProperty->Enum->GetNameStringByValue(ByteProperty->GetPropertyValue(ValueAddress)));
            }
            else
            {
 ...
 */
bool FSekiroAnimGraphIRLuaWriter::WriteModule(
    const FSekiroAnimBlueprintIR& Blueprint,
    FString& OutLuaText,
    TArray<FSekiroAnimIRDiagnostic>& OutDiagnostics)
{
    using namespace SekiroAnimGraphIRLuaWriterPrivate;
    OutLuaText.Reset();
    OutDiagnostics.Reset();
    FSekiroAnimBlueprintIR Canonical = Blueprint;
    USekiroAnimGraphIRLibrary::Canonicalize(Canonical);
    if (!USekiroAnimGraphIRLibrary::Validate(Canonical, OutDiagnostics)) return false;

    FString IRText;
    if (!WriteStruct(*FSekiroAnimBlueprintIR::StaticStruct(), &Canonical, 0, IRText))
    {
    ...
    OutLuaText = TEXT("-- Lua 类型：动画蓝图编译描述模块。此文件由 SekiroAnimBlueprintExt 生成，请勿手工修改。\n")
        ...
        TEXT("---@return table ir 可由 Sekiro AnimGraph IR Importer 直接读取的纯数据表。\n")
