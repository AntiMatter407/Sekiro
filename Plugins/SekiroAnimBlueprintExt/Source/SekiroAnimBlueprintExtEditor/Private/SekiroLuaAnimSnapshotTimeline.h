...
DECLARE_DELEGATE_OneParam(FOnSekiroLuaAnimSnapshotSelected, int32);

/** 可缩放、可平移并按时间选择真实快照的 Slate 时间轴。 */
class SSekiroLuaAnimSnapshotTimeline final : public SLeafWidget
{
public:
    SLATE_BEGIN_ARGS(SSekiroLuaAnimSnapshotTimeline)
        : _Frames(nullptr)
    {
    }
        SLATE_ARGUMENT(const TArray<TSharedPtr<FSekiroLuaAnimSnapshotFrame>>*, Frames)
        SLATE_EVENT(FOnSekiroLuaAnimSnapshotSelected, OnSnapshotSelected)
    SLATE_END_ARGS()

    void Construct(const FArguments& InArgs);
    void SetFrames(const TArray<TSharedPtr<FSekiroLuaAnimSnapshotFrame>>* InFrames);
...
private:
    ...
    const TArray<TSharedPtr<FSekiroLuaAnimSnapshotFrame>>* Frames = nullptr; // 查看器拥有的有序帧数组
    FOnSekiroLuaAnimSnapshotSelected OnSnapshotSelected; // 点选真实帧后的通知
