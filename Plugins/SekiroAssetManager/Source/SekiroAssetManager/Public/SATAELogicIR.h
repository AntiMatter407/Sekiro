...
#include "SATAELogicIR.generated.h"
...
enum class ESKJumpTableAction : uint8
{
    ...
    // ── 补充高频 JT ID ─────────────────────────────────────────
    JTID_16,                // 出现 82 次，特殊攻击变体标记
    JTID_24,                // 出现 20 次，未知
    JTID_39,                // 出现 107 次，姿态相关
    JTID_54,                // 出现 14 次，未知
    JTID_56,                // 出现 143 次，反击/格挡反击
    JTID_69,                // 出现 29 次，未知
    JTID_72,                // 出现 133 次，特定武器动作
    JTID_95,                // 出现 28 次，未知
    JTID_110,               // 出现 14 次，未知
    JTID_125,               // 出现 1 次，未知
    JTID_126,               // 出现 26 次，未知
    ...
    JTID_132,               // 出现 4 次，未知
    JTID_136,               // 出现 29 次，未知
    JTID_140,               // 出现 13 次，与义手相关
    JTID_141,               // 出现 132 次，与义手相关
    JTID_143,               // 出现 71 次，未知
    JTID_145,               // 出现 119 次，镜头震动
    JTID_146,               // 出现 41 次，镜头震动
    JTID_148,               // 出现 121 次，慢动作/时间控制
    JTID_149,               // 出现 15 次，未知
    JTID_150,               // 出现 38 次，未知
    JTID_151,               // 出现 77 次，未知
    JTID_155,               // 出现 14 次，未知
...
// ── ABIR: 单个动画的完整逻辑 ──────────────────────────────────
USTRUCT(BlueprintType)
struct FSAAnimationLogicIR
{
    GENERATED_BODY()

    UPROPERTY() int32 AnimID = 0;
    UPROPERTY() FString AnimName;                         // 由 AnimationNameMap 翻译
    UPROPERTY() FString AnimPrefix;               // 动画资产前缀 (a000, a010, a200...) 从 TAE 文件名推导

    // 事件分类
    UPROPERTY() TArray<FSATAEEventIR> AllEvents;
    UPROPERTY() TArray<FSACancelWindowIR> CancelWindows;   // JumpTable提取的取消窗口
    UPROPERTY() TArray<FSAAttackHitboxIR> AttackHitboxes;  // 攻击盒
    UPROPERTY() TArray<FSASpEffectIR> SpEffects;           // 状态效果
    UPROPERTY() TArray<FSASEventIR> SoundEvents;           // 音效
    UPROPERTY() TArray<FSAFFXEventIR> FFXEvents;           // 视觉特效
...
// ── ABIR: 完整状态机定义 ─────────────────────────────────────
USTRUCT(BlueprintType)
struct FSAStateMachineIR
{
    GENERATED_BODY()

    UPROPERTY() FString StateMachineName;
    UPROPERTY() TArray<FSAAnimationLogicIR> AnimStates;
    UPROPERTY() TArray<FSASMTransitionIR> Transitions;
