...
- 用户没有明确说明测试方式时，默认只执行与修改范围对应的编译验证。
- 默认禁止启动 PIE、模拟玩家输入或执行其他运行时功能测试。
- 只有用户明确要求“PIE 测试”“运行测试”或指定具体运行场景时，才允许启动 PIE 并执行对应验证。
- C++ 修改使用 UBT 编译对应目标或模块；Lua 动画蓝图等需要生成资产的修改只执行必要的生成与编译，不自动进入 PIE。
