...
**必需：**
- 读取 `Docs/gdd/game-concept.md` — **如果缺失则明确报错退出**：
  > "在 `Docs/gdd/game-concept.md` 未找到游戏概念。请先运行 `/brainstorm` 创建一个，然后再回来将其拆解为系统。"

**可选（如果存在则读取）：**
- 读取 `Docs/gdd/game-pillars.md` — 游戏支柱约束优先级和范围
- 读取 `Docs/gdd/systems-index.md` — 如果存在，**从上次中断处**继续（更新，而非从头重建）
- Glob `Docs/gdd/*.md` — 检查哪些系统 GDD 已经存在
...
询问："可以将系统索引写入 `Docs/gdd/systems-index.md` 吗？"
