...
FString FSKAnimationNameMap::Lookup(const FString& AnimId)
{
	const FString* Found = GetAnimationMap().Find(AnimId);
	return Found ? *Found : FString();
}

FString FSKAnimationNameMap::GetCategoryPrefix(const FString& AnimId)
{
...
FString FSKAnimationNameMap::Translate(const FString& RawName)
{
	// Pattern: "Sekiro_a000_XXXXXX" or "Sekiro_aXXX_XXXXXX"
	// Extract the last 6 digits as the animation ID
	...
	if (LastUnderscore >= 0 && (RawName.Len() - LastUnderscore - 1) == 6)
	{
		...
		if (bAllDigits)
		{
			...
			if (!Category.IsEmpty())
			{
				// Extract the prefix part (e.g. "a000" from "Sekiro_a000_201030")
				FString Prefix;
	...
	// No recognizable ID pattern — return as-is
	return RawName;
}
