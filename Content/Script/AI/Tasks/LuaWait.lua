...
---@type table<USekiroLuaBehaviorTreeTask, LuaWaitTaskState>
local TaskStates = setmetatable({}, {
    __mode = "k",
})

---开始一次等待；Configuration 可直接填写秒数字符串，缺失或无效时采用一秒。
---@param task USekiroLuaBehaviorTreeTask 当前运行时 Task 实例，用作隔离状态的稳定弱键。
...
---@return string result 返回 InProgress，使行为树继续调用 Tick。
...
---@return string result 未到时返回 InProgress，到达时返回 Succeeded。
...
---@return string result 返回 Succeeded，通用宿主会同步完成 Abort。
